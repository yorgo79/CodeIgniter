...
class CI_DB_cubrid_driver extends CI_DB {
	...
	public function __construct($params)
	{
		...
		if (function_exists('cubrid_connect') === FALSE)
		{
			$this->is_supported = FALSE;

			$error = 'The CUBRID PHP extension was not found on this system.';
	 ...
	 *
	 * Note: Persistent DB connection are supported natively in the CUBRID
	...
	public function db_connect($persistent = FALSE)
	{
		...
		elseif (preg_match('/^CUBRID:[^:]+(:[0-9][1-9]{0,4})?:[^:]+:([^:]*):([^:]*):(\?.+)?$/', $this->dsn, $matches))
		{
